(async () => {
  const config = window.PATCHMASON_CONFIG;
  ...
  root.innerHTML = `<div class="shell"><header class="top"><div class="brand"><i></i>Patchmason Cloud</div>${token ? '<button id="signout" class="button alt">Sign out</button>' : `<a class="button" href="${loginUrl}">Sign in</a>`}</header><section class="hero"><span class="eyebrow">PRODUCTION DEBUGGING, WITH CONSENT</span><h1>Reports with the context to fix them.</h1><p>Manage projects, generate a public SDK key, and triage reviewed reports. Patchmason never grants unrestricted access to customer accounts.</p></section><section class="grid"><article class="card"><span class="eyebrow">01 — PROJECTS</span><h2>Connect an app</h2><p>Create a project, select approved context fields, and copy its SDK key.</p></article><article class="card"><span class="eyebrow">02 — INBOX</span><h2>Triage reports</h2><p>Search, assign and send a reviewed report to your issue tracker.</p></article><article class="card"><span class="eyebrow">03 — CONTROLS</span><h2>Keep data bounded</h2><p>Set redaction, retention and workspace access before accepting evidence.</p></article></section><section class="reports"><h2>Your report inbox</h2>${reportRows}</section></div>`;
